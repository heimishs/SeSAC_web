...
io.on("connection", (socket) => {
  ...
  //   방이 없을때 {socket.id} 정보만 들어와있음

  socket.on("join", (roomname) => {
    // 2. 서버에서 방열기
    // join(): 같은 방에 들어가 있는 사람끼리 통신할 수 있도록
    socket.join(roomname); // roomname으로 된 방 열기
    socket.room = roomname; //다른곳 에서도 roomname을 확인 할 수있도록 정보 추가

    // 3-1. 입장 메세지 모두에게 보내기 server > client로 보냄
    // io.to(roomname).emit("userjoin", `${socket.id}님 입장`);

    // 3-2. 입장 메세지 모두에게 보내기 server > client로 보냄
    // broadcast: 내가 제와됨
    socket.broadcast.to(roomname).emit("userjoin", `${socket.id} 님 입장`);
  });

  // 6. client > server, 전체 메세지 보내기
  socket.on("message", (msg) => {
    console.log("클라이언트 메세지:", msg);
